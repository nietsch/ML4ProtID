...
def main():
    ...
    # Register path to XTandem! executable, needed by OpenMS tool XTandemAdapter
    model.add(
        "xtandem_path",
        required=True,
        type="string",
        is_list=False,
        description="Path to XTandem! executable"
    )

    # Register path to XTandemAdapter executable in order to run it with os.system
    model.add(
        "xtandemadapter_path",
        required=True,
        type="string",
        is_list=False,
        description="Path to XTandemAdapter executable"
    )
    ...
    xtandemadapter_path = arg_dict["xtandemadapter_path"]
    ...
    protein_ids, peptide_ids = xtandem_algorithm(searchfile, database, xtandem_path, xtandemadapter_path, hits_per_spec)
    ...
    perc_protein_ids, perc_peptide_ids = run_percolator(xtandem_res_add_vals_file, perc_path, percadapter_path)
...
def xtandem_algorithm(searchfile: str, database: str, xtandem_path: str, xtandemadapter_path: str, hits_per_spec: int):
    # Run XTandem! using XTandemAdapter, store protein and peptide ids

    xtandem_outfile = "xtandem_results.idXML"
    xtandem_outfile_addedfeats = "xtandem_results_addedfeats.idXML"
    ...
    psmfeats_command = "PSMFeatureExtractor -in " + xtandem_outfile + " -out " + xtandem_outfile_addedfeats
    os.system(psmfeats_command)

    protein_ids = []
    peptide_ids = []
    IdXMLFile().load(xtandem_outfile_addedfeats, protein_ids, peptide_ids)
